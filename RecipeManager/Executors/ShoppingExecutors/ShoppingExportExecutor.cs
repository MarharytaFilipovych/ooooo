...
public class ShoppingExportExecutor(
    IRecipeStorage recipeStorage, 
    IStorage<Plan> planStorage,
    IEventPublisher eventPublisher) : ICommandExecutor<ShoppingExportCommand>
{
    public ExecuteResult Execute(ShoppingExportCommand command)
    {
        ...
        var itemsCount = PrintShoppingList(plans, recipeMap);
        
        eventPublisher.Publish(new ShoppingExportedEvent(itemsCount));
    ...
    private static int PrintShoppingList(List<Plan> plans, Dictionary<string, Recipe> recipes)
    {
